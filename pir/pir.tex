...
% $Id: pir.tex,v 1.17 2005/06/09 09:36:52 keulsn Exp $
...
Person im Bildausschnitt aller 200 Bildsequenzen. Im anderen Szenario
variiert die Anzahl der Personen. In den folgenden Diagrammen ist die
tatsächliche Anzahl der Personen im Bild pro Bildsequenz angegeben, die
...
Im Rahmen dieser Fachstudie sollte keine neue Technologie werden,
sondern vorhandene Möglichkeiten der OpenCV Bibliothek evaluiert
werden.  Deshalb wurden zur Untersuchung die Beispielprogramme von
OpenCV herangezogen und soweit erweitert, dass verwertbare
Messergebnisse produziert werden.  Die Semantik der untersuchten
Verfahren Verfahren wurde nach Überzeugung der Autoren nicht
verändert.
...
In der ersten Spalte steht der Name des Bilds, in der zweiten die
...
von Feature-Points ersetzt werden.  Das Ergebnis ist das Programm
pyramid.c es akzeptiert als Parameter die Datei mit Feature-Point
Koordinaten sowie einen Schwellwert, ab dem eine Bewegung von
Feature-Points als reale Bewegung im Bild interpretiert wird.

Um mehrere Bewegungen zählen zu können, wird die Menge der
Feature-Points in einzelne Feature-Point Gruppen unterteilt.  In jedem
Schritt wird jede Gruppe in der eine Bewegung stattgefunden hat
gezählt.  Davon erhoffen sich die Autoren eine Personenzählung
...
wird von dem Beispielprogramm motempl.c implementiert.  Dieses
...
Anzahld der gepufferten Bilder und ein Bereich reeller Zahlen
...
die jeweils 200 Bildsequenzen pro Szenario angewendet. Dabei wurde pro
Bildsequenz von dem jeweiligen Algorithmus die Anzahl der erkannten
Personen (Bewegungsfelder) ausgegeben, die in den Diagrammen im Anhang
im Detail angegeben ist.

\subsection{Optischer Fluss}

Aufgrund nahezu identischer Resultate der oben angegebenen Algorithmen
CalcOpticalFlowHS, CalcOpticalFlowLK, CalcOpticalFlowBM und der eigenen
Implementierung, wurden diese Art von Algorithmen in einem Test
zusammengefasst. Da sich die genannten Algorithmen nicht parametrisieren
lassen, wurde nur ein Test pro Szenario durchgeführt. Die genauen
Resultate sind im Anhang ersichtlich.
Die Güte und Zuverlässigkeit dieser Algorithmen wurde in Beziehung zu den
tatsächlichen Personen im Bild in eine absolute Receiver-Operator Charakteristik
umgerechnet, die wie folgt pro Szenario ausgefallen ist.

\begin{center}
\begin{figure}
\includegraphics[width=7cm,angle=270]{impl/roc-optical-flow-1.log.ps}
\caption{Receiver Operator Charakteristik, erstes Szenario, Optische Fluss Algorithmen}
\end{figure}

\begin{figure}
\includegraphics[width=7cm,angle=270]{impl/roc-optical-flow-2.log.ps}
\caption{Receiver Operator Charakteristik, zweites Szenario, Optische Fluss Algorithmen}
\end{figure}
\end{center}

Aus den Charakteristiken ist ersichtlich, dass die Optischen
Fluss-Algorithmen keine Falsch-Positiv Treffer im ersten Szenario
...
Optische Fluss Algorithmen können also lediglich zur Existenzaussage
von Bewegungen in Betracht gezogen werden, für diese Aussagen sind sie
...
\begin{figure}[h]
    \begin{center}
        ...
        6 & 1s & 4 & 0.05 & 0.5\\ \hline
        7 & 2s & 4 & 0.05 & 0.5\\ \hline
        8 & 3s & 4 & 0.05 & 0.5\\ \hline
        9 & 5s & 4 & 0.05 & 0.5\\ \hline
        10& 10s & 4 & 0.05 & 0.5\\ \hline
        11& 1s & 8 & 0.05 & 0.5\\ \hline
        12& 2s & 8 & 0.05 & 0.5\\ \hline
        13& 3s & 8 & 0.05 & 0.5\\ \hline
        14& 5s & 8 & 0.05 & 0.5\\ \hline
        15& 10s & 8 & 0.001 & 1.0\\ \hline
        16& 1s & 2 & 0.001 & 1.0\\ \hline
        17& 2s & 2 & 0.001 & 1.0\\ \hline
        18& 3s & 2 & 0.001 & 1.0\\ \hline
        19& 5s & 2 & 0.001 & 1.0\\ \hline
        20& 10s & 2 & 0.001 & 1.0\\ \hline
        16& 1s & 2 & 0.001 & 3.0\\ \hline
        17& 2s & 2 & 0.001 & 3.0\\ \hline
        18& 3s & 2 & 0.001 & 3.0\\ \hline
        19& 5s & 2 & 0.001 & 3.0\\ \hline
        20& 10s & 2 & 0.001 & 3.0\\ \hline
        \end{tabular}
...
Bildsequenzen einen niedrigen Durchsatz pro Sekunde liefern (im Mittel
...
geliefert. Deshalb wurde für die Erstellung einer Receiver Operator
Charakteristik der Durchlauf 19 verwendet.

\begin{center}
\begin{figure}
\includegraphics[width=7cm,angle=270]{impl/roc-motion-1.log.ps}
\caption{Receiver Operator Charakteristik, erstes Szenario, SegmentMotion}
\end{figure}

\begin{figure}
\includegraphics[width=7cm,angle=270]{impl/roc-motion-2.log.ps}
\caption{Receiver Operator Charakteristik, zweites Szenario, SegmentMotion}
\end{figure}
\end{center}

Wie aus den Receiver Operator Charakteristiken ersichtlich, liefert
der SegmentMotion Algorithmus keine Falsch-Postiv Treffer im ersten
...
Feature-Points haben auf den vorliegenden Bildsequenzen eher keine positive
...
Die Receiver Operator Charakteristik wurde basierend auf Testdurchlauf 7
erstellt und ist in folgenden Diagrammen ersichtlich.

\begin{center}
\begin{figure}
\includegraphics[width=7cm,angle=270]{impl/roc-pyramid-1.log.ps}
\caption{Receiver Operator Charakteristik, erstes Szenario, CalcOpticalFlowPyrLK}
\end{figure}

\begin{figure}
\includegraphics[width=7cm,angle=270]{impl/roc-pyramid-2.log.ps}
\caption{Receiver Operator Charakteristik, zweites Szenario, CalcOpticalFlowPyrLK}
\end{figure}
\end{center}
...
liefern die einfachen optischen Fluss-Algorithmen.
