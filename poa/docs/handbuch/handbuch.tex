...
\begin{itemize}
\item QT 3.0
\item C++ Compiler: VisualStudio 6/.NET, alternativ MinGW gcc 2.95 oder h"oher
\end{itemize}

\item Linux auf allen Rechnerplattformen mit QT und X-Windows \newline
Systemanforderungen:
\begin{itemize}
\item QT 3.0
...
Dieser Abschnitt untergliedert sich in die Installation von releasten
POA Versionen unter Linux und in die Installation von Entwicklersnapshots
aus dem CVS.\par

\subsection{Installation von POA 1.0}

Die Installation von POA Releases, wie POA 1.0, ist kommandozeilenbasiert
und wird durch ein Installationsskript gesteuert. Vor der Installation muss zunächst
ein Terminal, z.B. {\it xterm}, gestartet werden.\par

\subsubsection{POA Release dekomprimieren}
Im Terminal wechselt man zunächst in das Verzeichnis, wo sich das POA
Release in komprimierter Form (z.B. \textbf{poa-1.0.tar.gz}) befindet.
...
Sollte noch kein POA Release heruntergeladen worden sein, kann dies
durch Besuch der POA Website auf {\it http://poa.berlios.de/} und dort
unter dem Menüpunkt \textbf{Download} nachgeholt werden. Alternativ kann POA
auch wie folgt kommandobasiert heruntergeladen werden.

\begin{verbatim}
$ cd /home/hans/downloads/
$ wget http://download.berlios.de/poa/poa-1.0.tar.gz
$ ls
poa-1.0.tar.gz
\end{verbatim}

Als nächster Schritt muss das POA Release dekomprimiert werden. Dies wird
durch folgenden Befehl durchgeführt, der auch den Fortschritt des 
Dekomprimierungsvorgangs darstellt:

\begin{verbatim}
$ tar xvzf poa-1.0.tar.gz
...
$ ls
poa-1.0          poa-1.0.tar.gz
\end{verbatim}

\subsubsection{POA Kompilierung vorbereiten}
Nach der Dekomprimierung existiert ein neues Verzeichnis
\textbf{poa-1.0}, in das zur Installation gewechselt werden muss.

\begin{verbatim}
$ cd poa-1.0
\end{verbatim}

In diesem Verzeichnis befinden sich viele C++ Source Dateien sowie ein
Installationsskript. Da POA Releases als Source ausgeliefert werden,
muss POA zunächst kompiliert werden. Hierzu muss sichergestellt werden,
dass einige Umgebungsvariablen für QT3 richtig gesetzt sind und die
ausführbaren QT3-Tools in der {\it PATH} Umgebungsvariablen verfügbar
sind. Hierzu ist es notwendig zu ermitteln, wo QT auf dem System
installiert ist. Um das zu bestimmen kann man folgenden Befehl
verwenden:

\begin{verbatim}
$ su
# find / -name 'qobject.h'
/usr/qt3/include/qobject.h
\end{verbatim}

Dies bedeutet, dass sich QT im Verzeichnis {\it /usr/qt3} befindet.
Sollte eine Ausgabe wie:

\begin{verbatim}
/usr/include/qobject.h
\end{verbatim}

erscheinen, so ist QT im Verzeichnis {\it /usr} installiert. Wenn eine
Ausgabe wie:

\begin{verbatim}
/usr/qt2/include/qobject.h
/usr/qt3/include/qobject.h
/usr/qt4/include/qobject.h
\end{verbatim}

erscheint, so ist für POA nur {\it /usr/qt3} relevant, da POA für QT
3.0 entwickelt wurde.\par
Nachdem nun der Ort der QT-Installation bestimmt wurde, werden zwei
Umgebungsvariablen gesetzt, wobei die Variable {\it QTDIR} auf das
ermittelte QT-Verzeichnis gesetzt wird.
...
Durch diese Umgebungsvariablen kann nun die Kompilierung von POA
beginnen.

\subsubsection{POA kompilieren}
Um POA zu kompilieren werden nun folgende Befehle eingegeben.
...
Die Kompilierung kann je nach System einige Zeit dauern, sollte jedoch
spätestens nach 15 Minuten beendet sein.\par
Nachdem die Kompilierung beendet ist, sollte sichergestellt werden, das
eine ausführbare Datei namens {\it poa} im aktuellen Verzeichnis existiert:

\begin{verbatim}
$ ls -l poa
-rwxr-xr-x  1 hans  users  8417818 Mar  5 09:29 poa
\end{verbatim}

Wenn dies der Fall ist, kann POA bereits aus dem aktuellen Verzeichnis
gestartet werden ({\it ./poa} eingeben). Falls eine Installation in ein
benutzerdefiniertes Verzeichnis gewünscht ist, kann aber auch das
Installationsskript verwendet werden.

\subsubsection{POA in benutzerdefiniertes Verzeichnis installieren}
Zur Installation in ein benutzerdefiniertes Verzeichnis wird folgender Befehl
eingegeben (sofern POA in ein globales Verzeichnis wie z.b. /usr
installiert werden soll, muss man Superuser (root) Rechte besitzen).
...
Die Ausführung des Installationsskriptes ohne Argumente installiert POA
standardmäßig nach {\it /usr/poa}, wenn ein anderer Installationspfad
gewünscht ist, kann man folgende Syntax abwandeln:

\begin{verbatim}
$ su (optional)
# ./install.sh -p /opt/local/
\end{verbatim}

In diesem Fall wird POA in das Verzeichnis {\it /opt/local/poa}
installiert.\par
Damit ist die Installation von POA bereits abgeschlossen. Sollte POA in
ein globales Verzeichnis installiert worden sein, kann das aktuelle
Verzeichnis wieder entfernt werden.

\subsection{Installation eines Entwicklersnapshots aus dem CVS}

Um einen Entwicklersnapshot von POA zu installieren, sind einige zum
vorherigen Abschnitt abweichende Dinge zu beachten.

\subsubsection{Auschecken aus dem CVS}

Entwicklersnapshots müssen aus dem CVS von POA ausgecheckt werden,
hierzu werden folgende Befehle verwendet:

\begin{verbatim}
$ cd /home/hans/projects
$ cvs -d:pserver:anonymous@cvs.poa.berlios.de:/cvsroot/poa login
$ cvs -z3 -d:pserver:anonymous@cvs.poa.berlios.de:/cvsroot/poa co poa
$ ls
poa     ...
\end{verbatim}

Sollte eine Passwortabfrage erscheinen, so ist diese einfach mit
\textbf{ENTER} zu bestätigen. Nach erfolgtem Auschecken existiert nun
ein Verzeichnis {\it poa} im aktuellen Verzeichnis. Sobald POA
ausgecheckt wurde, kann der Entwicklersnapshot wie folgt auf dem
aktuellen Stand halten werden:

\begin{verbatim}
$ cd /home/hans/projects/poa
$ cvs up -AdP
\end{verbatim}

\subsubsection{Abweichungen zur Installation eines Releases}

Die Verzeichnisstruktur eines Entwicklersnapshots weicht etwas von der
Verzeichnisstruktur eines POA Releases ab, da Entwicklersnapshots noch
weitere Verzeichnisse enthalten, die in POA Releases nicht mit released
werden, z.B. dieses Handbuch im \TeX-Source unter {\it
poa/docs/handbuch}.\par
Um einen POA Entwicklersnapshot zu installieren, muss in das folgende
...
Dieses Verzeichnis enthält alle Dateien in der selben
Verzeichnisstruktur, die in einem POA Release released werden. Um einen
Entwicklersnapshot zu installieren, ist das Vorgehen identisch zum
Unterabschnitt \textbf{'POA Kompilierung vorbereiten'} aus dem vorherigen
Abschnitt.

\section{Installation unter Windows}

Für die Dekomprimierung der Programmdateien wird eine Komprimierungsprogram
wie WinZIP, PowerArchiver oder Ähnliches benötigt.

\subsubsection{POA Release dekomprimieren}
Das Release ist entweder in .tar.gz oder in .zip Form vorhanden.
Der Inhalt davon sollte in ein beliebiges Verzeichnis dekomprimiert
werden.

Sollte noch kein POA Release heruntergeladen worden sein, kann dies
durch Besuch der POA Website auf {\it http://poa.berlios.de/} und dort
unter dem Menüpunkt \textbf{Download} nachgeholt werden.

\subsubsection{POA Kompilierung vorbereiten}
Nach der Dekomprimierung sollte eine Eingabeaufforderung geöffnet (Start -
Programme - Zubehör - Eingabeaufforderung oder alternativ Start - Ausführen -
\textbf{cmd} eingeben - OK) und in das Verzeichnis mit den Dateien gewechselt 
werden. Dies geht z.B. mit den Befehlen
...
dass einige Umgebungsvariablen für QT3 richtig gesetzt sind und die
ausführbaren QT3-Tools in der {\it PATH} Umgebungsvariablen verfügbar
sind. Bei einer korrekten Installation dürfte dies als gegeben
vorauszusetzen sein.

\subsubsection{POA kompilieren}
Um POA mit VisualStudio zu kompilieren werden nun folgende Befehle 
eingegeben.
...
spätestens nach 15 Minuten beendet sein.\par
Nachdem die Kompilierung beendet ist, sollte sichergestellt werden, das
eine ausführbare Datei namens {\it poa.exe} im aktuellen Verzeichnis 
existiert:
...
\subsubsection{POA in benutzerdefiniertes Verzeichnis installieren}
Zur Installation in das Verzeichnis c:$\backslash$poa 
...
Soll das Programm unter ein anderes Verzeichnis als c:$\backslash$ 
installiert werden, so kann dies dem Programm als Parameter mitgeteilt 
werden:
...
In diesem Fall wird POA in das Verzeichnis 
{\it c:$\backslash$programme$\backslash$poa} installiert.\par
Damit ist die Installation von POA bereits abgeschlossen. Sollte POA in
ein globales Verzeichnis installiert worden sein, kann das aktuelle
Verzeichnis wieder entfernt werden.
