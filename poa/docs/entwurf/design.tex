...
%% Entwurf
%% $Id: design.tex,v 1.16 2004/06/16 11:55:21 squig Exp $
...
abgeleitet. Die Methode \m{createView(GridCanvas*)} erzeugt für
...
Pin hinzugefügt bzw. wieder entfernt. Bei Aufruf der Methode
...
Die nächste Verfeinerung der Vererbungshierarchie ermöglicht die
Spezialisierung auf CPUs und Muxes, die es ermöglichen, Verbindungen
zwischen Blöcken zu multiplexen. Ein CPU-Block (\cn{CpuModel}) erhält
die neuen Felder \m{autoRuntime}, \m{cpuId} und \m{source}.  Diese
Felder ermöglichen die automatische Laufzeitdauerberechnung, die
...
Mapping von Pins auf Pins mit unterschiedlichen Bitbreiten ermöglicht.

Instanzen der Klasse \cn{PinModel} repräsentieren die einzelnen Pins
...
geschieht über einen Aufruf der Methode \m{attach(PinModel*
  connectTo)}. Wurde eine Verbindung erstellt, gibt die Methode
\m{connected()} gibt den Pin des anderen Blocks zurück. Ausserdem
...
\m{PinType::EPISODIC} annehmen. Über den Konstruktor wird dem Pin der
Vaterblock mitgeteilt. Dieser kann über \m{parent()} abgefragt werden.
...
wurden. In der derzeitige Implementierung hören nur die
